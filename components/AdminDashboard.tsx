...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  const handleAddDept = (e: React.FormEvent) => {
    ...
    // Explicitly assigning all required fields
    const deptObject: Department = { 
      id: `dept-${Date.now()}`, 
      name: newDept.name, 
      code: newDept.code, 
      description: newDept.description || 'Standard academic unit infrastructure.',
      instituteId: newDept.instituteId 
    };
    
    onAddDepartment(deptObject);
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            return (
              <div key={inst.id} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {instDepts.map(dept => (
                    <button key={dept.id} onClick={() => setSelectedDeptId(dept.id)} className="bg-white p-6 rounded-2xl border-2 border-slate-200 hover:border-indigo-300 transition-all text-left group">
                      <div className="flex justify-between items-start mb-4">
                        <div className="bg-indigo-50 text-indigo-600 p-3 rounded-xl group-hover:bg-indigo-600 group-hover:text-white transition-all"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg></div>
                  ...
                  <button onClick={() => handleOpenDeptModal(inst.id)} className="border-2 border-dashed border-slate-300 rounded-2xl p-8 hover:bg-white hover:border-indigo-400 transition-all flex flex-col items-center justify-center bg-slate-100 min-h-[160px]">
                    ...
                    <p className="font-black text-slate-600 text-[10px] uppercase tracking-widest text-center">Provision Unit to {inst.name.split(' ')[0]}</p>
                  </button>
                </div>
              </div>
            );
        ...
        <div className="animate-in fade-in slide-in-from-right-4 duration-500 space-y-12 pb-20">
          <div className="bg-indigo-600 rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden">
            ...
            <div className="relative z-10 max-w-2xl">
               <h2 className="text-4xl font-black uppercase tracking-tighter mb-4">Neural Cloud Pipeline</h2>
               <p className="text-indigo-100 font-bold leading-relaxed mb-8">
                 Ensure your academic database is synchronized across the LADTEM network. Validating your Vercel connection prevents data fragmentation.
               </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-white p-10 rounded-[3rem] border-2 border-slate-200 shadow-xl space-y-8 relative overflow-hidden">
              ...
              <h3 className="text-xl font-black uppercase tracking-tighter text-slate-900">Bridge Configuration</h3>
              <div className="space-y-6">
                 <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Vercel Blob Link</label>
                    <div className="flex gap-2">
                      <input 
                        type="text" 
                        value={tempBlobUrl} 
                        onChange={(e) => setTempBlobUrl(e.target.value)} 
                        placeholder="https://...blob.vercel-storage.com/data.json" 
                 ...
                 <div className="grid grid-cols-2 gap-4">
                   <button 
                      onClick={() => {
                        ...
                        alert("🚀 Bridge Authorized! Node is now syncing with Vercel Cloud.");
                      }} 
                    ...
                    >
                     <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944" /></svg>
                     <span>Authorize</span>
                   </button>
                   {blobConfigUrl && (
                     <a 
            ...
            <div className="bg-slate-900 p-10 rounded-[3rem] text-white space-y-8 relative overflow-hidden shadow-2xl">
              <h3 className="text-xl font-black uppercase tracking-tighter text-emerald-400">Node Sync Guide</h3>
              <div className="space-y-4">
                 <div className="flex items-start space-x-4">
                    <div className="bg-white/10 p-2 rounded-lg text-emerald-400 font-black">01</div>
                    <p className="text-[11px] font-bold text-slate-300 leading-tight">Your data is stored under your Unique Node ID (e.g. LADTEM-OFFICE).</p>
                 </div>
                 <div className="flex items-start space-x-4">
                    <div className="bg-white/10 p-2 rounded-lg text-emerald-400 font-black">02</div>
                    <p className="text-[11px] font-bold text-slate-300 leading-tight">Copy your Vercel Blob URL into the field to the left on every device you use.</p>
                 </div>
                 <div className="flex items-start space-x-4">
                    <div className="bg-white/10 p-2 rounded-lg text-emerald-400 font-black">03</div>
                    <p className="text-[11px] font-bold text-slate-300 leading-tight">The data you see at the "View Cloud Data" link is exactly what every authorized device will sync.</p>
                 </div>
              </div>
            </div>
          </div>
      ...
      {activeTab === 'approvals' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in duration-500">
           {pendingUsers.map(user => (
             <div key={user.id} className="bg-white p-8 rounded-[2.5rem] border-2 border-slate-200 shadow-xl space-y-6">
                <div className="flex items-center space-x-4">
                  ...
                  <div><p className="text-lg font-black text-slate-900">{user.name}</p><p className="text-[10px] text-slate-400 font-black uppercase">Pending Approval</p></div>
                </div>
           ...
           {pendingUsers.length === 0 && <div className="col-span-full py-20 text-center text-slate-400 font-bold uppercase italic text-xs">No pending applications.</div>}
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md flex items-center justify-center z-[100] p-4">
          <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl p-8">
            <div className="flex justify-between items-center mb-8">
              <div>
                <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Deploy Unit</h2>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Node ID: {institutes.find(i => i.id === newDept.instituteId)?.name}</p>
              </div>
